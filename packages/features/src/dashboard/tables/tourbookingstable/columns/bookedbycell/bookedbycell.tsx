...
export function BookedByCell({ row }: TourBookingCellProps) {
      ...
      >
        ...
        <DropdownMenuItem>
          <Link
            ...
            className="truncate max-w-52 flex items-center gap-2"
          >
            <Mail size={18} /> {(booking.email || bookedBy?.email) ?? "No email"}
          </Link>
        </DropdownMenuItem>

        <DropdownMenuItem>
          <Link href={`tel:${booking.phone || bookedBy?.phone}`} className="truncate max-w-52 flex items-center gap-2">
            <Phone size={18} /> {(booking.phone || bookedBy?.phone) ?? "No phone"}
          </Link>
        </DropdownMenuItem>

        <DropdownMenuItem>
          <span className="truncate max-w-52 flex items-center gap-2">
            <IdCard size={18} /> {(booking.cnic || bookedBy?.cnic) ?? "No CNIC"}
          </span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
