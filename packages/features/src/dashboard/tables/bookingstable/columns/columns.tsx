...
export const bookingsColumns: ColumnDef<BookingWithUserListItemResponseDto>[] = [
  ...
  },
  {
    ...
    header: "Dropoff Date",
    size: 140,
    cell: ({ row }) => format(new Date(row.original.dropoffDate), "dd MMM yyyy"),
  },
  {
    ...
    size: 220,
    cell: ({ row }) => (
      <span className="block max-w-[200px] truncate" title={row.original.pickupAddress}>
  ...
  },
  {
    ...
    size: 220,
    cell: ({ row }) => (
      <span className="block max-w-[200px] truncate" title={row.original.dropoffAddress}>
