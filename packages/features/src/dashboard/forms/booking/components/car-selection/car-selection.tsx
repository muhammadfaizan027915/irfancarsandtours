...
export function CarSelection({ onSelect, selectedCarIds }: CarSelectionProps) {
  ...
  const fetchCars = React.useCallback(async (pageNum: number, isInitial: boolean = false) => {
    if (isInitial) setLoading(true);
    else setLoadingMore(true);

    try {
      const result = await searchCarsForBooking({ search: debouncedSearch, limit: PAGE_SIZE, page: pageNum });
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className="w-full justify-between shadow-none min-h-11"
          >
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0" align="start">
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            {loading && page === 1 && <div className="p-4 text-sm text-center text-muted-foreground">Searching...</div>}
            {!loading && cars.length === 0 && <div className="p-4 text-sm text-center text-muted-foreground">No cars found.</div>}
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isSelected ? "opacity-50 cursor-not-allowed bg-accent/50" : "hover:bg-accent hover:text-accent-foreground"
                  )}
                  onClick={() => {
                    if (!isSelected) {
                      onSelect(car);
                ...
                >
                  ...
                  <div className="flex flex-col min-w-0">
                      <span className="truncate">{car.brand} {car.name} ({car.model})</span>
                      <span className="text-xs text-muted-foreground truncate">Starting Price: Rs. {car.startingPrice}</span>
                  </div>
                </div>
              );
            })}
