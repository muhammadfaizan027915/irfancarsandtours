...
export function UserSelection({
  ...
  const fetchUsers = React.useCallback(async (pageNum: number, isInitial: boolean = false) => {
    if (isInitial) setLoading(true);
    else setLoadingMore(true);

    try {
      const result = await searchCustomersForBooking({ name: debouncedSearch, limit: PAGE_SIZE, page: pageNum });
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            {selectedUser ? (
              ...
              <span className="text-muted-foreground">Select existing customer or create new...</span>
            )}
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0" align="start">
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            ...
            {!loading && users.length === 0 && search && <div className="p-4 text-sm text-center text-muted-foreground">No customers found.</div>}
