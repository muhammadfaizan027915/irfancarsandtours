...
export function CustomersFilterBar() {
  ...
  const cnicValue = getSearchParams("cnic")?.[0];
  ...
  return (
    <Card className="py-4 shadow-none rounded-xl gap-2">
      <CardHeader className="px-4 h-8 flex flex-row gap-2 justify-between items-center">
        <h3 className="font-semibold text-lg">Filter Customers</h3>
        {hasFilters && (
          <Button
      ...
      <CardContent className="px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        ...
        />
        <TextFilter label="CNIC" paramName="cnic" placeholder="Search by CNIC" />
        <DateRangeFilter label="Joined On" />
      </CardContent>
    </Card>
  );
}
